...
//
//https://api.openweathermap.org/data/1.0/zip?zip=95035,us&appid=ea532800f653719e0a32716ac930d92c
...
interface WeatherService {
    ...
    suspend fun getWeatherForGeoLocation(@Query("lat") lat: Double, @Query("lon") long: Double, @Query("appid") appId: String, @Query("units") units: String): WeatherData

    @GET("geo/1.0/direct")
    suspend fun getLatLongForLocation(@Query("q") location: String, @Query("appid") appId: String, @Query("limit") limit: Int): List<GeoLocationData>

    @GET("geo/1.0/zip")
    suspend fun getLatLongForZip(@Query("zip") zip: String, @Query("appid") appId: String): GeoLocationData
}